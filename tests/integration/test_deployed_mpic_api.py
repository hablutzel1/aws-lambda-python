...
class TestDeployedMpicApi:
    ...
    @pytest.mark.parametrize('domain_or_ip_target, purpose_of_test', [
        ('dname-permit.deny.basic.caatestsuite.com', 'Tests handling of a DNAME when a permissible CAA record exists at DNAME target'),
        ('cname-permit-sub.deny.basic.caatestsuite.com', 'Tests handling of a CNAME when a permissible CAA record exists at parent of CNAME target')
    ])
    # fmt: on
    def api_should_return_is_valid_false_for_do_not_issue_caa_test_suite_for_rfc_8659(
